...
| `set-device-config` | Override device config (`--text-scale`, `--bold-text`, `--platform-brightness`, `--reset`). Requires the app to wrap its root widget in `MarionetteDeviceConfig` — see [MCP Tools](./mcp-tools.md#device-configuration). |
