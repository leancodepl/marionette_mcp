...
| `take_screenshots` | Capture screenshots of all active views, returned as base64 PNGs. Pass `inline: false` to save them into the [session directory](./session-reports.md) instead and get back their paths — cheaper when you're citing evidence, not looking at it. |
