...
- Add session reports: `connect` opens a fresh, titled (`session_title`) `.marionette/sessions/` directory — one run always produces exactly one directory, never resumed; every tool call is appended to `steps.md` (tool, selector, outcome — `enter_text` values redacted); `take_screenshots` gains a save-only `inline: false` mode; `disconnect` returns the session path with a reminder to write `report.md`. Old sessions are pruned automatically. The `marionette` CLI opens the same kind of session directory via `--session`/`--session-dir` and logs one step per invocation. The report contract (evidence-backed findings, the `Tested:` line rendered from `steps.md`) lives in the "Reporting what you found" section of `marionette-flutter-drive-app`. See [Session Reports](./docs/session-reports.md)
