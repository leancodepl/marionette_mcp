...
- Add `skills/marionette-drive-flutter-apps/SKILL.md`, an Agent Skill covering the full tool/CLI surface and Marionette best practices
- Add `marionette_flutter-drive-app`, an Agent Skill shipped inside the `marionette_flutter` package, installable via `dart run skills@ get` (Dart's [package skills](https://dart.dev/tools/pub/package-skills) convention) or `npx skills add leancodepl/marionette_mcp` — see the new "Agent Skill" section in the README
